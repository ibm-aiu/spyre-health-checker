...
// socket and returns the socket path plus a stop function.
...
var _ = Describe("CardHealthClient", func() {
	Describe("CollectForSlots", func() {
		...
		It("maps healthy response to DEVICE_STATE_ONLINE", func() {
			client := NewCardHealthClient(sockPath)
		...
		It("skips slots when the socket does not exist", func() {
			client := NewCardHealthClient("/nonexistent/path.sock")
			// grpc.NewClient is lazy — the error surfaces on the first RPC call
			// which times out; CollectForSlots skips the slot and returns empty.
			states, err := client.CollectForSlots([]string{"0000:01:00.0"})
			Expect(err).NotTo(HaveOccurred())
			Expect(states).To(BeEmpty())
		})
	})
